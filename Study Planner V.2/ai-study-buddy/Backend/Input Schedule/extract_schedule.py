...
def get_user_schedule_image(db, user_id):
    ...
    try:
        ...
        if user_study_plan and "scheduleImage" in user_study_plan:
            ...
            print(f"No schedule image found in study_plans for user ID: {user_id}")
            if user_study_plan:
                print(f"Study plan fields: {list(user_study_plan.keys())}")
...
def main():
    try:
        user_id = os.getenv("USER_ID") or input("Enter user ID: ")
        print(f"Processing schedule for user ID: {user_id}")
        db = get_mongodb_connection()
        schedule_data = extract_schedule_from_db(db, user_id)
        if schedule_data:
            print("\nFinal Schedule:")
            print(json.dumps(schedule_data, indent=2))
            with open(f"schedule_{user_id}.json", "w") as f:
                json.dump(schedule_data, f, indent=2)
            print(f"Schedule saved to schedule_{user_id}.json")
        else:
